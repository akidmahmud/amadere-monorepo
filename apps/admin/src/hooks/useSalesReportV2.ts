...
export interface OrderLine { key: string; name: string; qty: number; price: number; disc: number; gross: number; weight: number; unitWeight?: number; unitCost?: number | null; costOk?: boolean; net?: number; cogs?: number | null }
...
export function useReportSettings(enabled = true) {
  return useQuery({ queryKey: [...KEY, "settings"], queryFn: () => proxyFetch<SettingsResponse>(`${BASE}/settings`), enabled });
}

/** Every order matching the filters, unpaged — for the Export CSV. */
export function fetchAllOrders(f: ReportFilters) {
  return proxyFetch<OrdersResponse>(`${BASE}/orders?${reportParams(f, { all: "true" })}`);
}
