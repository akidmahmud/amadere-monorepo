...
import { usePatchWholesaleCustomer, type WholesaleCustomer } from "@/hooks/useWholesale";
...
export function WholesaleCustomersTable({
  ...
  return (
    <div className="overflow-hidden rounded-card border shadow-[0_1px_2px_rgba(20,40,25,.05)]" style={{ background: "#fff", borderColor: "#e5ebe6" }}>
        ...
        {customers.map((c) => (
          <MobileRecordCard
            ...
            fields={[
              ...
              { label: "Status", value: c.crmStatus ? STATUS_LABEL[c.crmStatus] : "" },
              { label: "Top product", value: c.topProduct ?? "" },
              { label: "Assigned", value: c.assignedAdminName ?? "" },
            ]}
            actions={
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={() => actions.onOrder(c)} className="inline-flex h-8 items-center gap-1.5 rounded-sm border border-border px-3 text-xs font-semibold text-text">
                  <Icon name="add" size={14} /> New Order
                </button>
                <button type="button" onClick={() => actions.onEdit(c)} className="inline-flex h-8 items-center rounded-sm border border-border px-3 text-xs font-semibold text-text">
                  Edit
                </button>
              </div>
            }
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="border-separate border-spacing-0" style={{ minWidth: 3500, width: "100%" }}>
          <thead>
            <tr>
              <TH sticky={1} style={{ minWidth: 200 }}>Name</TH>
              <TH>Fav</TH>
              <TH>Actions</TH>
              <TH>B-Day</TH>
          ...
          <tbody>
            {customers.map((c) => (
              <WholesaleCustomerRow key={c.id} customer={c} staff={staff} {...actions} />
            ))}
          </tbody>
        </table>
      </div>
...
function WholesaleCustomerRow({
  ...
  onOrder,
  onEdit,
}: { customer: WholesaleCustomer; staff?: { id: number; name: string }[] } & Actions) {
  ...
  const textCell = (value: string, set: (v: string) => void, original: string | null, save: (v: string) => void, placeholder: string, width: number) =>
    editing ? (
      <input
        value={value}
  ...
  const dateCell = (value: string | null, save: (v: string | null) => void) =>
    editing ? (
      <input type="date" defaultValue={toDateInputValue(value)} onChange={(e) => save(e.target.value || null)} className={cellInputClass} style={{ width: 140 }} />
    ) : (
      <ReadOnlyText value={formatDate(value)} width={140} />
    );

  const iconButton = "grid h-[29px] w-[29px] place-items-center rounded-[8px] border border-transparent hover:border-[#e5ebe6]";

  return (
    <tr className="[&:hover>td]:bg-[#f7fbf8]">
      <td className={td} style={{ ...tdStyle, position: "sticky", left: 0, zIndex: 6, boxShadow: "6px 0 8px -6px rgba(20,40,25,.14)" }}>
        {editing ? (
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            onBlur={() => name.trim() && name !== c.name && update.mutate({ name: name.trim() })}
          ...
          <span className="block w-[170px] truncate px-1.5 font-bold" style={{ color: INK }} title={name}>
            {name}
          </span>
        )}
        <button type="button" onClick={() => onView(c)} className="mt-[3px] block px-1.5 text-[0.66rem] font-medium hover:underline" style={{ color: FAINT }}>
          #WS-{c.id}
          {!c.isActive ? " · Inactive" : ""}
      ...
      <td className={td} style={tdStyle}>
        <div className="flex items-center gap-[5px]">
          <button type="button" onClick={() => onView(c)} aria-label="View full history" title="View Full History" className={iconButton} style={{ color: FAINT }}>
            {eyeIcon}
          </button>
          <button
            type="button"
          ...
          <button type="button" onClick={() => onOrder(c)} aria-label="New order" title="New Order" className={iconButton} style={{ color: FAINT }}>
            {cartIcon}
          </button>
          <button type="button" onClick={() => onEdit(c)} aria-label="Edit details" title="Edit details (credit, landmark, note…)" className={iconButton} style={{ color: FAINT }}>
            {settingsIcon}
          </button>
        </div>
      </td>
      <td className={td} style={tdStyle}>{dateCell(c.dob, (dob) => update.mutate({ dob }))}</td>
      <td className={td} style={tdStyle}>
        {textCell(address, setAddress, c.address, (v) => update.mutate({ address: v }), "Add address...", 220)}
      </td>
      <td className={td} style={tdStyle}>
        {editing ? (
          <div style={{ width: 150 }}>
      ...
      <td className={td} style={tdStyle}>
        {editing ? (
          <div style={{ width: 150 }} onBlur={() => thana !== (c.thana ?? "") && update.mutate({ thana })}>
            <ThanaAutocomplete district={district} value={thana} onChange={setThana} />
          </div>
        ) : (
          <ReadOnlyText value={thana} placeholder="এখানে থানা টাইপ করুন" width={150} />
        )}
      </td>
      <td className={td} style={tdStyle}>
        {/* A shop's landline is allowed, as in the customer form — only an empty phone is refused. */}
        {textCell(phone, setPhone, c.phone, (v) => v.trim() && update.mutate({ phone: v.trim() }), "Phone", 130)}
      </td>
      <td className={td} style={tdStyle}>
        {textCell(email, setEmail, c.email, (v) => update.mutate({ email: v }), "Email", 190)}
      </td>
      ...
      <td className={td} style={{ ...tdStyle, fontWeight: 700, color: INK }}>
        ...
        {Number(c.due) > 0 && <span className="block text-[0.66rem]" style={{ color: RED }}>{money(c.due)} due</span>}
      </td>
      <td className={td} style={tdStyle}>
      ...
      <td className={td} style={tdStyle}>{formatDate(c.createdAt)}</td>
      <td className={td} style={{ ...tdStyle, fontWeight: 700, color: INK }}>{formatDate(c.lastOrderAt)}</td>
      <td className={td} style={tdStyle}>{dateCell(c.nextCallTarget, (nextCallTarget) => update.mutate({ nextCallTarget }))}</td>
      <td className={td} style={tdStyle}>
      ...
      <td className={td} style={tdStyle}>
        <select value={c.hasNewOrder ? "yes" : "no"} onChange={(e) => update.mutate({ hasNewOrder: e.target.value === "yes" })} className={cellSelectStyle} style={cellSelectStyleObj}>
          <option value="no">No</option>
          <option value="yes">Yes</option>
        </select>
      </td>
      <td className={td} style={{ ...tdStyle, color: FAINT, fontWeight: 500 }}>
        {c.hasNewOrder ? dateCell(c.newOrderAt, (newOrderAt) => update.mutate({ newOrderAt })) : "—"}
      </td>
      <td className={td} style={tdStyle}>
        <select
          value={c.priority ?? "MEDIUM"}
          onChange={(e) => update.mutate({ priority: e.target.value as WholesaleCustomer["priority"] })}
          className={cellSelectStyle}
      ...
      <td className={td} style={tdStyle}>
        <select
          ...
          onChange={(e) => update.mutate({ crmStatus: e.target.value as WholesaleCustomer["crmStatus"] })}
          className={cellSelectStyle}
      ...
      <td className={td} style={tdStyle}>
        <select
          value={c.behaviour ?? ""}
          onChange={(e) => update.mutate({ behaviour: (e.target.value || null) as WholesaleCustomer["behaviour"] })}
          className={cellSelectStyle}
          style={cellSelectStyleObj}
        >
          <option value="">—</option>
      ...
      <td className={td} style={tdStyle}>
        {textCell(feedback, setFeedback, c.customerFeedback, (v) => update.mutate({ customerFeedback: v }), "Add feedback...", 190)}
      </td>
      <td className={td} style={tdStyle}>
        {textCell(amaderFeedback, setAmaderFeedback, c.amaderFeedback, (v) => update.mutate({ amaderFeedback: v }), "Add note...", 190)}
      </td>
      <td className={td} style={tdStyle}>
        {textCell(family, setFamily, c.familyDetails, (v) => update.mutate({ familyDetails: v }), "Add details...", 190)}
      </td>
      <td className={td} style={tdStyle}>
        {textCell(reason, setReason, c.purchaseReason, (v) => update.mutate({ purchaseReason: v }), "Reason...", 150)}
      </td>
      {[c.fScore, c.mScore].map((score, i) => (
        <td key={i} className={td} style={tdStyle}>
          <span className="inline-flex h-6 min-w-[26px] items-center justify-center rounded-[7px] px-[7px] text-[0.7rem] font-extrabold" style={{ background: scoreBadgeStyle(score).bg, color: scoreBadgeStyle(score).color }}>
            {score}
          </span>
        </td>
      ))}
      ...
      <td className={td} style={tdStyle}>
        <div className="flex items-center gap-1.5">
          {textCell(fbUrl, setFbUrl, c.facebookProfileUrl, (v) => update.mutate({ facebookProfileUrl: v }), "Profile URL...", 130)}
