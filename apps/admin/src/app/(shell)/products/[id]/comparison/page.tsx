...
import { ComparisonFields } from "@/components/products/ComparisonFields";
  ...
  return (
    <Card className="max-w-2xl">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-sm font-semibold text-text">Comparison — {product.translations[0]?.name ?? product.slug}</p>
        <Link href={`/products/${productId}`} className="text-xs font-semibold text-brand-500 hover:underline">
          ← Back to product
        </Link>
      </div>
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <ComparisonFields form={form} />
