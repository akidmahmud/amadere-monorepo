...
export default function EditCategoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const displayName = category.translations.find((t) => t.locale === "EN")?.name ?? category.translations[0]?.name ?? category.slug;
    ...
    <div className="flex flex-col gap-4">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex items-start gap-2.5 rounded-inner border border-[#d8e6fc] bg-brand-50 px-3.5 py-2.5 text-[0.75rem] font-semibold text-brand-600">
          ...
          <span>
            A category is a place in your storefront&apos;s browsing tree — keep both languages accurate so it reads correctly
            for every customer.
            <br />
            <span lang="bn">একটি ক্যাটেগরি আপনার স্টোরফ্রন্টের ব্রাউজিং তালিকার একটি জায়গা — উভয় ভাষা সঠিক রাখুন যাতে প্রতিটি গ্রাহক সঠিকভাবে দেখতে পারেন।</span>
          </span>
        </div>
