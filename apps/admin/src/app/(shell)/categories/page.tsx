...
import { useCategories, useDeleteCategory, useUpdateCategory, type AdminCategory } from "@/hooks/useCategories";
...
const starIcon = (filled: boolean) => (
  <svg width="17" height="17" viewBox="0 0 24 24" fill={filled ? "#f5a623" : "#dfe5e0"}>
...
function CategoryRow({ category: c, parentName, onDelete }: { category: AdminCategory; parentName: string | undefined; onDelete: (c: AdminCategory) => void }) {
    ...
    <tr className="border-b border-[#f1f5fa] last:border-b-0 hover:bg-[#fafcfe]">
      <td className="px-2.5 py-3 align-middle">
        {c.imageUrl ? (
          ...
          <img src={c.imageUrl} alt="" className="h-[42px] w-[42px] rounded-[9px] border border-border object-cover" />
        ) : (
          <div className="grid h-[42px] w-[42px] place-items-center rounded-[9px] border border-border bg-surface-2 text-base">🗂️</div>
        )}
      </td>
      <td className="px-2.5 py-3 align-middle">
      ...
      <td className="px-2.5 py-3 align-middle whitespace-nowrap text-[0.78rem] font-semibold text-text">{parentName ?? "—"}</td>
      <td className="px-2.5 py-3 align-middle whitespace-nowrap text-[0.78rem] font-bold text-text">{c.productCount ?? 0}</td>
      <td className="px-2.5 py-3 align-middle">
        <button
          type="button"
      ...
      <td className="px-2.5 py-3 align-middle">
        <div className="flex items-center gap-1.5">
          <Link href={`/categories/${c.id}`} aria-label="Edit" className="grid h-[30px] w-[30px] place-items-center rounded-[8px] bg-brand-50 text-brand-500 hover:bg-brand-100">
            {editIcon}
          </Link>
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm leading-relaxed text-secondary">
        This is where your storefront&apos;s category tree lives — every category customers browse products through. Publish or
        archive one, nest it under a parent, and mark it Featured to highlight it on the homepage.
        <br />
        <span lang="bn">
          আপনার স্টোরফ্রন্টের ক্যাটেগরি তালিকা এখান থেকে পরিচালনা করা হয় — গ্রাহকরা এই ক্যাটেগরিগুলোর মাধ্যমেই পণ্য খুঁজে পান। যেকোনো
          ক্যাটেগরি প্রকাশ বা আর্কাইভ করুন, প্যারেন্ট ক্যাটেগরির অধীনে রাখুন, এবং হোমপেজে হাইলাইট করতে ফিচার্ড হিসেবে চিহ্নিত করুন।
        </span>
      </p>
      ...
      <div className="rounded-card border border-border bg-surface p-[18px_18px_14px] shadow-card">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <span className="text-[0.76rem] font-semibold text-muted">{filtered.length} of {categories?.length ?? 0} categories</span>
          <Link
            href="/categories/new"
        ...
        {isLoading ? (
          ...
          <div className="mt-3 overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr>
                  {["Image", "Category", "Parent", "Products", "Featured", "Status", "Actions"].map((h, i) => (
                    <th
                      key={h}
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-2.5 py-8 text-center text-sm text-muted">
                      {categories?.length === 0 ? "No categories yet." : "No categories match these filters."}
                    </td>
                  </tr>
                )}
                {filtered.map((c) => (
                  <CategoryRow key={c.id} category={c} parentName={nameFor(c.parentId)} onDelete={setDeleteTarget} />
                ))}
              </tbody>
            </table>
          </div>
