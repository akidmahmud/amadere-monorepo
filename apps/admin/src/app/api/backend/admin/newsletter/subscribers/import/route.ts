...
// Dedicated route, not the [...path] catch-all — same reason as
// apps/admin/src/app/api/backend/admin/media/route.ts and
// .../net-profit/blocker/import/route.ts: the catch-all reads the body via
// `req.text()`, which corrupts a multipart CSV upload.
...
export async function POST(req: NextRequest) {
  ...
  async function call(token: string | undefined) {
    return fetch(`${BACKEND_URL}/api/v1/admin/newsletter/subscribers/import`, {
      method: "POST",
