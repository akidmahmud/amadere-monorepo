...
/**
 * The invoice configuration, without any page chrome.
 *
 * ONE form, rendered in two places: Settings > Invoices, and the Wholesale
 * page's own Invoice Settings tab. Wholesale and retail invoices render from
 * the same template and the same company details, so giving wholesale its own
 * copy of this screen would let the two drift apart and print two different
 * companies from one business.
 */
export function InvoiceSettingsForm() {
    ...
    update.mutate(
      {
        ...
        stampImageUrl: draft.stampImageUrl || null,
  ...
  return (
    ...
    <Card className="flex flex-col gap-4">
      ...
      {/* Fed the draft, not the saved settings — the point is to see what an
          unsaved change will print. */}
      <InvoicePreview
        draft={{
          ...draft,
          companyLogoUrl: draft.companyLogoUrl || null,
