  ...
  return (
    <div className="rounded-xl border border-emerald-800/20 bg-gradient-to-b from-white via-white to-emerald-50/20 p-6 shadow-sm">
      ...
      <p className="mb-5 text-xs font-semibold text-emerald-900/60">
        Whole-product edits only — variant, cross-sell, and SKU/stock/price
        sub-edits aren&apos;t attributed to a specific product ID in the audit
        log yet.
      </p>
      {isLoading && (
      ...
      <div className="flex flex-col gap-2">
        {data?.items.map((entry) => (
          <div
            key={entry.id}
            className="flex items-center justify-between gap-3 rounded-lg border border-emerald-800/15 bg-white p-3 shadow-xs transition-all hover:border-amber-400/40"
          >
