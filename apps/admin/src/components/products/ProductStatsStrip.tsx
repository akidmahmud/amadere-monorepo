...
const TILES: {
  ...
  },
  {
    ...
    icon: icon(
      <>
        <rect x="3" y="3" width="18" height="18" rx="4" />
        <path d="m14 8.5 1.5 1.5L10 15.5H8.5V14Z" />
      </>,
    ),
  },
  {
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
      {TILES.map((t) => (
        <div
          key={t.key}
          className="flex items-center justify-between gap-3 rounded-card border border-border bg-surface p-[15px_18px] shadow-card"
        >
          <div>
            <div className="text-[0.76rem] font-semibold text-secondary">
              {t.label}
            </div>
            <div className="mt-1 text-[1.3rem] font-extrabold text-text">
              {stats ? stats[t.key] : "—"}
            </div>
          </div>
          <div
            className="grid h-10 w-10 flex-none place-items-center rounded-inner"
