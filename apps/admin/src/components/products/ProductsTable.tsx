...
const STATUS_PILL: Record<string, string> = {
  ...
  // Blue, not green: it is live for staff but not on the storefront, and
  // colouring it like PUBLISHED would hide exactly that difference.
  ADMIN_ONLY: "bg-[#e6f0fe] text-[#2563eb]",
};
...
function SeoRing({ score }: { score?: number }) {
  if (score === undefined)
    return <span className="text-[0.7rem] font-bold text-muted">—</span>;
  const r = 16;
  ...
  return (
    <div className="relative h-[38px] w-[38px]">
      ...
      <span className="absolute inset-0 flex items-center justify-center text-[0.66rem] font-extrabold text-text">
        {score}
      </span>
    </div>
  );
...
export function ProductsTable({
  ...
  const categoryName = new Map(
    (categories ?? []).map((c) => [c.id, c.translations[0]?.name ?? c.slug]),
  );
  ...
  const [deleteTarget, setDeleteTarget] = useState<
    { id: number; name: string } | "bulk" | null
  >(null);

  async function confirmDelete() {
    if (deleteTarget === "bulk") {
      await Promise.all(
        [...selected].map((id) => deleteProduct.mutateAsync(id)),
      );
      setSelected(new Set());
      setBulkAction("");
      router.refresh();
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(
      () =>
        onFiltersChange({ ...filters, q: searchInput || undefined, page: 1 }),
      350,
    );
  ...
  async function applyBulkAction() {
    ...
    if (bulkAction === "delete") {
      ...
      await Promise.all(
        ids.map((id) =>
          fetch(`/api/backend/admin/products/${id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ status } as never),
          }),
        ),
      );
    } else if (bulkAction === "in_stock" || bulkAction === "out_of_stock") {
      const stockStatus =
        bulkAction === "in_stock" ? "IN_STOCK" : "OUT_OF_STOCK";
      await Promise.all(
        ids.map((id) =>
          fetch(`/api/backend/admin/products/${id}`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ stockStatus } as never),
          }),
        ),
      );
    }
  ...
  function exportHref() {
    ...
    for (const [k, v] of Object.entries(filters)) {
      if (v === undefined || v === "" || k === "page" || k === "pageSize")
        continue;
    ...
    <div className="min-w-0 rounded-card border border-border bg-surface p-[18px_18px_14px] shadow-card">
      <div className="flex min-w-0 flex-wrap items-center gap-2.5">
        ...
        <select
          ...
          className="h-[38px] min-w-[130px] rounded-inner border border-border bg-surface px-2.5 text-[0.75rem] font-semibold text-secondary outline-none"
        >
        ...
        {bulkAction && selected.size > 0 && (
          <button
            ...
            className="h-[38px] rounded-inner bg-brand-500 px-3 text-[0.76rem] font-bold text-white hover:bg-brand-600"
          >
            Apply ({selected.size})
          </button>
        )}
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr>
              ...
              {[
                ...
                <th
                  key={h}
                  className="whitespace-nowrap bg-[#f7f9fc] px-2.5 py-[11px] text-left text-[0.73rem] font-bold text-secondary"
                >
                  {h}
                </th>
              ))}
              <th className="rounded-r-[8px] bg-[#f7f9fc] px-2.5 py-[11px] text-left text-[0.73rem] font-bold text-secondary">
                Actions
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            {products.map((p) => {
              ...
              const variantStock = p.hasVariants
              ...
              const variantReserved = p.hasVariants
                ? p.variants.reduce((sum, v) => sum + v.reservedStock, 0)
                : p.reservedStock;
              const displayPrice = p.hasVariants
                ? (p.variants.find((v) => v.isDefault) ?? p.variants[0])?.price
                : p.price;
                ...
                <Fragment key={p.id}>
                  <tr className="border-b border-[#f1f5fa] last:border-b-0 hover:bg-[#fafcfe]">
                      ...
                      <div className="flex min-w-[190px] items-center gap-2">
                        ...
                        {thumb ? (
                          ...
                          <img
                            src={thumb.url}
                            alt=""
                            className="h-[46px] w-[46px] flex-none rounded-[9px] border border-border object-cover"
                          />
                        ) : (
                          <div className="grid h-[46px] w-[46px] flex-none place-items-center rounded-[9px] border border-border bg-surface-2 text-lg">
                            📦
                          </div>
                        )}
                        <div className="min-w-0">
                          <span className="line-clamp-2 block max-w-[180px] font-bold text-text">
                            {name}
                          </span>
                          {p.hasVariants && (
                            <span className="text-[0.68rem] font-semibold text-brand-500">
                    ...
                    <td className="px-2.5 py-3.5 align-middle whitespace-nowrap">
                      ...
                      <Pill
                        className={`mt-[5px] ${STOCK_PILL[p.stockStatus] ?? "bg-surface-2 text-secondary"}`}
                      >
                        {STOCK_LABEL[p.stockStatus] ?? p.stockStatus}
                      </Pill>
                      {variantReserved > 0 && (
                        <div className="mt-[3px] text-[0.68rem] font-semibold text-[#e0821c]">
                          {variantReserved} on hold &middot;{" "}
                          {variantStock - variantReserved} available
                        </div>
                      )}
                    </td>
                    <td className="px-2.5 py-3.5 align-middle whitespace-nowrap font-bold text-text">
                      ৳{displayPrice ?? "—"}
                    </td>
                    <td className="px-2.5 py-3.5 align-middle whitespace-nowrap">
                      <Pill
                        className={
                          STATUS_PILL[p.status] ?? "bg-surface-2 text-secondary"
                        }
                      >
                      ...
                      <td colSpan={9} className="px-2.5 py-3">
                        <table className="w-full border-collapse">
                          <thead>
                            <tr>
                              {[
                                ...
                                <th
                                  key={h}
                                  className="px-2.5 py-1.5 text-left text-[0.68rem] font-bold text-muted"
                                >
                                  {h}
                                </th>
                              ))}
                          ...
                          <tbody>
                            {p.variants.map((v) => (
                              <tr key={v.id}>
                                <td className="px-2.5 py-1.5 text-[0.75rem] font-bold text-text">
                                  {v.sku ?? `Variant #${v.id}`}
                                </td>
                                <td className="px-2.5 py-1.5 text-[0.75rem] font-semibold text-text">
                                  ...
                                  {v.reservedStock > 0 && (
                                    <span className="ml-1 font-semibold text-[#e0821c]">
                                      ({v.reservedStock} on hold &middot;{" "}
                                      {v.stock - v.reservedStock} available)
                                    </span>
                                  )}
                                </td>
                                <td className="px-2.5 py-1.5">
                                  <Pill
                                    className={
                                      STOCK_PILL[v.stockStatus] ??
                                      "bg-surface-2 text-secondary"
                                    }
                                  >
                                    {STOCK_LABEL[v.stockStatus] ??
                                      v.stockStatus}
                                  </Pill>
                                </td>
                                <td className="px-2.5 py-1.5 text-[0.75rem] font-semibold text-text">
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            ...
            .map((n, i) =>
              n === -1 ? (
                <span
                  key={`dots-${i}`}
                  className="px-1 text-[0.74rem] text-muted"
                >
                  …
                </span>
              ) : (
