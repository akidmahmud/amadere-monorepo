...
const inputClass =
  "h-9 min-w-0 flex-1 rounded-inner border border-border bg-surface px-2.5 text-[0.75rem] text-text outline-none focus:border-brand-500";

const STATUS_OPTIONS = [
  { value: "PUBLISHED", label: "Active" },
  { value: "DRAFT", label: "Draft" },
  // Sellable by staff, invisible to customers — worth filtering to, because
  // it is the one status where "live" and "on the storefront" differ.
...
export function ProductFilters({
  ...
  function set<K extends keyof AdminProductFilters>(
    key: K,
    ...
    <div className="rounded-card border border-border bg-surface p-[18px] shadow-card">
      <div className="mb-3.5 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={onReset}
          className="text-[0.76rem] font-bold text-brand-500 hover:underline"
        >
          Reset
        </button>
      </div>
      ...
      <div className="mb-[18px]">
        <div className="mb-2.5 text-[0.76rem] font-bold text-text">
          Category
        </div>
        <div className="flex flex-col gap-2">
          {categories?.map((c) => (
            <label
              key={c.id}
              className="flex cursor-pointer items-center gap-2 text-[0.76rem] font-semibold text-text"
            >
              <input
                type="checkbox"
            ...
            >
              <input
                ...
                onChange={() =>
                  set(
                    "stockStatus",
                    filters.stockStatus === s.value ? undefined : s.value,
                  )
                }
                className="h-[15px] w-[15px] accent-brand-500"
              />
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            set("brandId", e.target.value ? Number(e.target.value) : undefined)
          }
        >
