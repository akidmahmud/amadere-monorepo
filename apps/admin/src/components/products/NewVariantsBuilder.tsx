...
export function NewVariantsBuilder({
  attributes,
  variants,
  onChange,
  costPerItem,
  costPriceUnit,
}: NewVariantsBuilderProps) {
  return (
    <div>
      <span className="mb-2.5 block text-xs font-bold text-emerald-950">
        Defined Variants ({variants.length})
      </span>
      {attributes.length === 0 && (
        <p className="mb-3 text-xs font-medium text-emerald-900/60">
          Select at least one attribute above to define variants.
        </p>
      )}
      <div className="mb-3 flex flex-col gap-2">
        {variants.map((v, i) => {
          ...
          const profit =
            effectiveCost !== undefined ? v.price - effectiveCost : null;
          const saleProfit =
            effectiveCost !== undefined && v.salePrice != null
              ? v.salePrice - effectiveCost
              : null;
          return (
            <div
              key={i}
              className="flex items-center justify-between gap-3 rounded-xl border border-emerald-800/15 bg-gradient-to-r from-emerald-50/40 via-white to-amber-50/20 p-3 shadow-xs"
            >
              <div className="flex flex-wrap items-center gap-2 text-xs font-semibold text-emerald-950">
                <span className="font-extrabold text-emerald-900">
                  {labelFor(attributes, v.attributeValueIds)}
                </span>
                <span className="rounded-full bg-emerald-800/10 px-2.5 py-0.5 font-bold text-emerald-800">
                  ৳{v.price}
                </span>
                {v.salePrice != null && (
                ...
                {profit !== null && (
                  <span
                    className={`rounded-md px-2 py-0.5 text-[11px] font-bold ${profit < 0 ? "bg-rose-50 text-rose-700" : "bg-emerald-800 text-amber-300"}`}
                  >
                ...
                {saleProfit !== null && (
                  <span
                    className={`rounded-md px-2 py-0.5 text-[11px] font-bold ${saleProfit < 0 ? "bg-rose-50 text-rose-700" : "bg-emerald-800 text-amber-300"}`}
                  >
