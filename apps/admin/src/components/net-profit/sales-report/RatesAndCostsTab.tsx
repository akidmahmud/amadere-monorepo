  ...
  return (
    <div className="grid grid-cols-[minmax(0,1fr)] gap-3.5">
      {ro && (
        ...
        >
          Managers can view rates and costs. Only admins can change them.
        </div>
      )}
      {!ro && (dirty || save.isError) && (
        <div
          className="sticky top-2 z-10 flex items-center gap-3 rounded-lg border bg-white px-3 py-2 shadow-sm"
          style={{ borderColor: COLORS.line }}
        >
          <span className="text-sm">
            Unsaved changes to rates, fees or thresholds.
          </span>
          {save.isError && (
            <span className="text-sm" style={{ color: COLORS.loss }}>
          ...
          <button
            ...
            className="rounded-lg px-3.5 py-1 text-sm font-semibold text-white disabled:opacity-40"
      ...
      <Panel
        ...
        hint="Agreed charge = rate for the parcel weight + COD charge. Zones are your Shipping Zones. Change a value and save to see every report update."
      >
        <RateCardEditor
          s={draft}
      ...
      <Panel
        title="Product costs"
        hint="Each order uses the cost active on its order date, so adding a new cost never changes past reports."
      >
        <CostHistoryEditor canEdit={!ro} />
      </Panel>
      <Panel
        title="Other costs and alerts"
        hint="Payment fees apply to advance amounts. Enter your merchant rates."
      >
      ...
      {!ro && (
        <Panel
          title="Courier bills"
          hint="Upload the courier's per-parcel statement CSV. Parcels are matched by consignment ID; the billed charge replaces the rate-card estimate."
        >
