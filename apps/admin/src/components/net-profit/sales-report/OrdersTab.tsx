...
export function OrdersTab({ f, money, onExport }: TabProps) {
  ...
  useEffect(() => {
    const codPct = Object.fromEntries(
      Object.entries(settings.data?.settings.rates ?? {}).map(([k, r]) => [
        k,
        r.cod,
      ]),
    );
    // Two header rows (group labels + titles); columns P–V are the courier
    // block the owner's sheet shades light blue.
    onExport(
      "sales-report",
      async () =>
  ...
  const sorts: [string, string][] = [
    ["newest", "Newest first"],
    ["sales", "Highest net sales first"],
    ...(money
      ? ([
          ["contrib", "Lowest contribution first"],
          ["over", "Biggest courier overcharge first"],
        ] as [string, string][])
      : []),
  ];
  return (
    <div>
      <div className="mb-2.5 flex flex-wrap items-center gap-2.5">
        <input
          ...
          className="min-h-[34px] min-w-0 flex-1 rounded-[7px] border bg-white px-2 text-sm sm:min-w-[320px] sm:flex-none"
        ...
        <select
          ...
          className="min-h-[34px] rounded-[7px] border bg-white px-2 text-sm"
          style={{ borderColor: COLORS.line }}
        >
          {sorts.map(([k, l]) => (
      ...
      >
        {rows.length === 0 ? (
          <div className="p-7 text-center" style={{ color: COLORS.ink2 }}>
            No orders match these filters. Widen the date range or clear
            filters.
          </div>
        ) : (
          <table className="w-full border-collapse text-[14.5px]">
            <thead>
              <tr
                ...
                style={{ color: COLORS.muted, background: "#fbfcfb" }}
              >
                {[
                  "Order",
                  "Status",
                ...
                <th
                  className="border-b px-3 text-right font-medium"
                  style={{ borderColor: COLORS.line }}
                >
                  Net sales
                </th>
                {money &&
                  ["Delivery paid", "Courier charge", "Contribution"].map(
                    (h) => (
                      <th
                        key={h}
                        className="border-b px-3 text-right font-medium"
                ...
                return (
                  <Fragment key={c.o.id}>
                    <tr
                      ...
                      onClick={() => setOpen(isOpen ? null : c.o.id)}
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          setOpen(isOpen ? null : c.o.id);
                        }
                      }}
                      className="cursor-pointer align-top hover:bg-[#fbfcfb]"
                      style={
                        isOpen ? { background: COLORS.greenWash } : undefined
                      }
                    >
                      <td
                      ...
                      <td
                        className="min-w-[150px] border-b px-3 py-2"
                        style={{ borderColor: COLORS.line }}
                      >
                        {c.o.customer}
                        <div
                      ...
                      <td
                        className="min-w-[170px] border-b px-3 py-2"
                        style={{ borderColor: COLORS.line }}
                      >
                        {c.lines[0]
                          ? `${c.lines[0].name} × ${c.lines[0].qty}`
                          : "-"}
                        {c.lines.length > 1 && (
                          <div
                            className="text-[13px]"
                    ...
                    {isOpen && (
                      <tr>
                        <td
                          colSpan={cols}
                          className="border-b px-4 pb-[18px] pt-3.5"
                          style={{
                            background: "#fbfcfb",
                            borderColor: COLORS.line,
                          }}
                        >
                          <OrderDetail c={c} money={money} />
                        </td>
                      </tr>
                    )}
                  </Fragment>
