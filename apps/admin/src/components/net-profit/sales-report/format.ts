...
export const FLAG: Record<
  FlagKey,
  { label: string; tone: "loss" | "amber" | "info"; action: string }
> = {
  loss: {
    label: "Loss on order",
    tone: "loss",
    action:
      "Check price, discount and delivery charge before repeating this offer.",
  },
  low: {
    label: "Low contribution",
    tone: "amber",
    action:
      "Small or heavy orders: consider a minimum order or weight-based delivery fee.",
  },
  over: {
    label: "Courier overcharge",
    tone: "loss",
    action:
      "Raise with the courier and claim the difference in the next settlement.",
  },
  nocourier: {
    label: "Courier not set",
    tone: "amber",
    action: "Set the courier so the agreed rate can be checked.",
  },
  stuck: {
    label: "Stuck before shipping",
    tone: "amber",
    action: "Call the customer, then ship or cancel.",
  },
  nobill: {
    label: "Courier bill missing",
    tone: "amber",
    action:
      "Import the courier settlement so the real charge replaces the estimate.",
  },
  unconf: {
    label: "Unconfirmed product cost",
    tone: "info",
    action: "Confirm the cost in Rates and costs so profit is final.",
  },
  nocost: {
    label: "Product cost missing",
    tone: "loss",
    action:
      "Add the cost in Rates and costs; this order is left out of profit.",
  },
};

export const TAG_STYLE: Record<
  "loss" | "amber" | "info" | "plain",
...
export const courierLabel = (c: string | null) =>
  c ? (COURIER_LABEL[c] ?? c) : "Not set";
...
export const pc = (n: number) =>
  !Number.isFinite(n) ? "-" : (Math.round(n * 1000) / 10).toFixed(1) + "%";
