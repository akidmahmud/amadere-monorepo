...
export function ReportTable({
  ...
  const cell = (num?: boolean) =>
    `border-b px-3 py-2 align-top ${num ? "text-right tabular-nums whitespace-nowrap" : ""}`;
  if (rows.length === 0)
    return (
      <div
  ...
  return (
    ...
    >
      <table className="w-full border-collapse text-[14.5px]">
        <thead>
          <tr style={{ background: "#fbfcfb", color: COLORS.muted }}>
            {cols.map((c) => (
              <th
                key={c.h}
                className={`${cell(c.num)} text-[13.5px] font-medium ${c.num ? "" : "text-left"}`}
                style={{ borderColor: COLORS.line }}
              >
                {c.h}
              </th>
        ...
        {foot && (
          <tfoot>
            <tr className="font-semibold" style={{ background: "#fbfcfb" }}>
              {foot.map((v, j) => (
                <td
                  key={j}
                  className={cell(cols[j]?.num)}
