...
export function DailyChart({
  ...
  const max = Math.max(
    1,
    ...data.map((x) => Math.max(val(x), money ? (x.contrib ?? 0) : 0)),
  );
  ...
  return (
    <svg
      ...
      width="100%"
      role="img"
      aria-label="Daily net sales and contribution"
    >
      {[max, max / 2, 0].map((v) => (
        <g key={v}>
          ...
          <g key={x.date}>
            ...
            {money && (
              <rect
                ...
                y={Y(Math.max(0, c))}
                width={w * 0.56}
                height={Math.max(0, Y(Math.min(0, c)) - Y(Math.max(0, c)))}
                fill={c < 0 ? COLORS.loss : COLORS.gain}
                rx={2}
              >
