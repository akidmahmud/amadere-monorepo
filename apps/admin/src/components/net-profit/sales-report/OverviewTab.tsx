...
export function Panel({ title, hint, children }: { title: string; hint?: string; children: React.ReactNode }) {
  return (
    <section className="rounded-xl border bg-white px-[18px] py-4" style={{ borderColor: COLORS.line }}>
...
function Ledger({ s, basis }: { s: Summary; basis: string }) {
  ...
  return (
    <Panel title="Where the money went" hint={`${s.dN} delivered and ${s.rN} returned order(s), ${basis === "order" ? "booked" : "closed"} in this period`}>
      ...
      {s.missing > 0 && <div className="mt-2 text-[13px]" style={{ color: COLORS.loss }}>{s.missing} delivered order(s) are missing a product cost or a courier charge and are left out. See Exceptions.</div>}
