...
-- Seeds the 11 default email templates (base header/footer chrome, admin
-- password reset, and the ecommerce order-lifecycle + digital-download set)
-- into a fresh
...
-- The only delivery mechanism for a paid digital product: it carries the
-- buyer's private download link. can_disable = 'f' for that reason -- an
-- admin turning this off would silently leave every guest buyer with no way
-- to reach the file they paid for.
...
-- Sent instead of order_placed when every line on the order is DIGITAL.
-- A PDF has no delivery to promise and no COD to collect, so it needs its
-- own wording rather than the parcel-shaped order_placed.
